...
do
  ...
  gitsigns.setup {
    current_line_blame_opts = { delay = 300 }, -- default 1000ms feels sluggish when toggled deliberately
    signs = {
  ...
  -- [[ Sessions ]]
  --  Saves and restores your open files per directory, so `nvim` in a project
  --  picks up where you left off. `:SessionSearch` lists saved sessions.
  vim.pack.add { gh 'rmagatti/auto-session' }
  vim.opt.sessionoptions:append 'localoptions' -- keep per-buffer options (filetype, etc.) across a restore
